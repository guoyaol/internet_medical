...
public class AdminAdminController {
    ...
    public Object list(String username,
        ...
        List<InternetmedicalAdmin> adminList = adminService.querySelective(username, page, limit, sort, order);
        return ResponseUtil.okList(adminList);
    }

    private Object validate(InternetmedicalAdmin admin) {
    ...
    @PostMapping("/create")
    public Object create(@RequestBody InternetmedicalAdmin admin) {
        ...
        List<InternetmedicalAdmin> adminList = adminService.findAdmin(username);
        if (adminList.size() > 0) {
    ...
    public Object read(@NotNull Integer id) {
        InternetmedicalAdmin admin = adminService.findById(id);
        return ResponseUtil.ok(admin);
    }
    ...
    @PostMapping("/update")
    public Object update(@RequestBody InternetmedicalAdmin admin) {
        Object error = validate(admin);
        if (error != null) {
            return error;
        }
    ...
    @PostMapping("/delete")
    public Object delete(@RequestBody InternetmedicalAdmin admin) {
        Integer anotherAdminId = admin.getId();
        if (anotherAdminId == null) {
            return ResponseUtil.badArgument();
        }
        ...
        InternetmedicalAdmin currentAdmin = (InternetmedicalAdmin) currentUser.getPrincipal();
