...
import static org.linlinjava.internetmedical.admin.util.AdminResponseCode.*;
import static org.linlinjava.internetmedical.admin.util.AdminResponseCode.ORDER_PAY_FAILED;
...
public class AdminOrderService {
    ...
    public Object detail(Integer id) {
        InternetmedicalOrder order = orderService.findById(id);
        List<InternetmedicalOrderGoods> orderGoods = orderGoodsService.queryByOid(id);
     ...
     * 2. 管理员登录internetmedical管理后台点击退款操作进行订单状态修改和商品库存回库
     *
    ...
    public Object reply(String body) {
        ...
        // 目前只支持回复一次
        InternetmedicalComment comment = commentService.findById(commentId);
        if(comment == null){
            return ResponseUtil.badArgument();
        }
