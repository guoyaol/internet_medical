...
import org.linlinjava.internetmedical.db.service.InternetmedicalSearchHistoryService;
...
public class WxSearchController {
    ...
    @Autowired
    private InternetmedicalKeywordService keywordsService;
    @Autowired
    ...
    public Object index(@LoginUser Integer userId) {
        //取出输入框默认的关键词
        InternetmedicalKeyword defaultKeyword = keywordsService.queryDefault();
        //取出热闹关键词
        List<InternetmedicalKeyword> hotKeywordList = keywordsService.queryHots();

        List<InternetmedicalSearchHistory> historyList = null;
        if (userId != null) {
            //取出用户历史关键字
    ...
    public Object helper(@NotEmpty String keyword,
        ...
        List<InternetmedicalKeyword> keywordsList = keywordsService.queryByKeyword(keyword, page, limit);
        String[] keys = new String[keywordsList.size()];
        int index = 0;
        for (InternetmedicalKeyword key : keywordsList) {
