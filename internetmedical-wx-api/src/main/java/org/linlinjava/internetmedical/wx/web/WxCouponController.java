...
import org.linlinjava.internetmedical.wx.vo.CouponVo;
import org.linlinjava.internetmedical.wx.util.WxResponseCode;
...
public class WxCouponController {
    ...
    public Object list(@RequestParam(defaultValue = "1") Integer page,
        ...
        List<InternetmedicalCoupon> couponList = couponService.queryList(page, limit, sort, order);
    ...
    public Object mylist(@LoginUser Integer userId,
        ...
        List<InternetmedicalCouponUser> couponUserList = couponUserService.queryList(userId, null, status, page, limit, sort, order);
        List<CouponVo> couponVoList = change(couponUserList);
        return ResponseUtil.okList(couponVoList, couponUserList);
    }

    private List<CouponVo> change(List<InternetmedicalCouponUser> couponList) {
        List<CouponVo> couponVoList = new ArrayList<>(couponList.size());
        for(InternetmedicalCouponUser couponUser : couponList){
    ...
    public Object selectlist(@LoginUser Integer userId, Integer cartId, Integer grouponRulesId) {
        ...
        for (CouponVo cv : couponVoList) {
            InternetmedicalCoupon coupon = couponVerifyService.checkCoupon(userId, cv.getCid(), cv.getId(), checkedGoodsPrice, checkedGoodsList);
    ...
    public Object exchange(@LoginUser Integer userId, @RequestBody String body) {
        ...
        InternetmedicalCoupon coupon = couponService.findByCode(code);
        if(coupon == null){
