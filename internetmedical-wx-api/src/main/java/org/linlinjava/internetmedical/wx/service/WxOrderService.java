...
import org.linlinjava.internetmedical.core.express.ExpressService;
import org.linlinjava.internetmedical.core.express.dao.ExpressInfo;
import org.linlinjava.internetmedical.core.notify.NotifyService;
import org.linlinjava.internetmedical.core.notify.NotifyType;
...
import org.linlinjava.internetmedical.core.util.DateTimeUtil;
...
import org.linlinjava.internetmedical.db.util.OrderHandleOption;
import org.linlinjava.internetmedical.db.util.OrderUtil;
import org.linlinjava.internetmedical.core.util.IpUtil;
import org.linlinjava.internetmedical.wx.task.OrderUnpaidTask;
...
import static org.linlinjava.internetmedical.wx.util.WxResponseCode.*;
...
public class WxOrderService {
    ...
    @Autowired
    private InternetmedicalOrderGoodsService orderGoodsService;
    @Autowired
    private InternetmedicalAddressService addressService;
    @Autowired
    ...
    @Autowired
    private InternetmedicalCommentService commentService;
    @Autowired
    ...
    @Autowired
    private InternetmedicalAftersaleService aftersaleService;
    ...
    public Object list(Integer userId, Integer showType, Integer page, Integer limit, String sort, String order) {
        ...
        List<InternetmedicalOrder> orderList = orderService.queryByOrderStatus(userId, orderStatus, page, limit, sort, order);

        List<Map<String, Object>> orderVoList = new ArrayList<>(orderList.size());
        for (InternetmedicalOrder o : orderList) {
            ...
            InternetmedicalGroupon groupon = grouponService.queryByOrderId(o.getId());
            if (groupon != null) {
            ...
            List<InternetmedicalOrderGoods> orderGoodsList = orderGoodsService.queryByOid(o.getId());
    ...
    public Object detail(Integer userId, Integer orderId) {
        ...
        // 订单信息
        InternetmedicalOrder order = orderService.findById(userId, orderId);
        if (null == order) {
        ...
        List<InternetmedicalOrderGoods> orderGoodsList = orderGoodsService.queryByOid(order.getId());
    ...
    public Object submit(Integer userId, String body) {
        ...
        if (grouponRulesId != null && grouponRulesId > 0) {
            InternetmedicalGrouponRules rules = grouponRulesService.findById(grouponRulesId);
            //找不到记录
            if (rules == null) {
                return ResponseUtil.badArgument();
            }
            ...
            if (grouponLinkId != null && grouponLinkId > 0) {
                ...
                // （2）不允许参加自己开团的团购
                InternetmedicalGroupon groupon = grouponService.queryById(userId, grouponLinkId);
        ...
        // 收货地址
        InternetmedicalAddress checkedAddress = addressService.query(userId, addressId);
        ...
        InternetmedicalGrouponRules grouponRules = grouponRulesService.findById(grouponRulesId);
        if (grouponRules != null) {
            grouponPrice = grouponRules.getDiscount();
        }

        // 货品价格
        List<InternetmedicalCart> checkedGoodsList = null;
        if (cartId.equals(0)) {
            checkedGoodsList = cartService.queryByUidAndChecked(userId);
        } else {
            InternetmedicalCart cart = cartService.findById(cartId);
        ...
        for (InternetmedicalCart checkGoods : checkedGoodsList) {
        ...
        if (couponId != 0 && couponId != -1) {
            InternetmedicalCoupon coupon = couponVerifyService.checkCoupon(userId, couponId, userCouponId, checkedGoodsPrice, checkedGoodsList);
            if (coupon == null) {
                return ResponseUtil.badArgumentValue();
            }
        ...
        InternetmedicalOrder order = null;
        // 订单
        order = new InternetmedicalOrder();
        ...
        // 添加订单商品表项
        for (InternetmedicalCart cartGoods : checkedGoodsList) {
            // 订单商品
            InternetmedicalOrderGoods orderGoods = new InternetmedicalOrderGoods();
        ...
        for (InternetmedicalCart checkGoods : checkedGoodsList) {
            ...
            InternetmedicalGoodsProduct product = productService.findById(productId);
        ...
        //如果是团购项目，添加团购信息
        if (grouponRulesId != null && grouponRulesId > 0) {
            InternetmedicalGroupon groupon = new InternetmedicalGroupon();
            ...
            if (grouponLinkId != null && grouponLinkId > 0) {
                //参与的团购记录
                InternetmedicalGroupon baseGroupon = grouponService.queryById(grouponLinkId);
    ...
    public Object payNotify(HttpServletRequest request, HttpServletResponse response) {
        ...
        InternetmedicalOrder order = orderService.findBySn(orderSn);
        if (order == null) {
        ...
        //  支付成功，有团购信息，更新团购信息
        InternetmedicalGroupon groupon = grouponService.queryByOrderId(order.getId());
        if (groupon != null) {
            InternetmedicalGrouponRules grouponRules = grouponRulesService.findById(groupon.getRulesId());
            ...
            List<InternetmedicalGroupon> grouponList = grouponService.queryJoinRecord(groupon.getGrouponId());
            if (groupon.getGrouponId() != 0 && (grouponList.size() >= grouponRules.getDiscountMember() - 1)) {
                for (InternetmedicalGroupon grouponActivity : grouponList) {
                    grouponActivity.setStatus(GrouponConstant.STATUS_SUCCEED);
                    grouponService.updateById(grouponActivity);
                }

                InternetmedicalGroupon grouponSource = grouponService.queryById(groupon.getGrouponId());
    ...
    public Object goods(Integer userId, Integer orderId, Integer goodsId) {
        ...
        List<InternetmedicalOrderGoods> orderGoodsList = orderGoodsService.findByOidAndGid(orderId, goodsId);
        ...
        InternetmedicalOrderGoods orderGoods = orderGoodsList.get(0);
    ...
    public Object comment(Integer userId, String body) {
        ...
        InternetmedicalOrderGoods orderGoods = orderGoodsService.findById(orderGoodsId);
        if (orderGoods == null) {
            return ResponseUtil.badArgumentValue();
        }
        ...
        // 1. 创建评价
        InternetmedicalComment comment = new InternetmedicalComment();
    ...
    public void releaseCoupon(Integer orderId) {
        List<InternetmedicalCouponUser> couponUsers = couponUserService.findByOid(orderId);
        for (InternetmedicalCouponUser couponUser: couponUsers) {
            // 优惠券状态设置为可使用
