...
const staticService = axios.create({
  baseURL: 'http://localhost:9527', // api 的 base_url
...
staticService.interceptors.request.use(
  config => {
    ...
    if (store.getters.token) {
      // 让每个请求携带token-- ['X-Internetmedical-Admin-Token']为自定义key 请根据实际情况自行修改
      config.headers['X-Internetmedical-Admin-Token'] = getToken()
    }
    return config
  },
  error => {
...
staticService.interceptors.response.use(
  response => {
    ...
    console.log(staticService.interceptors.response)
